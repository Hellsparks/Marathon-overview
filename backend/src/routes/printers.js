...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(
    `INSERT INTO printers (name, host, port, api_key, bed_width, bed_depth, bed_height, filament_types, toolhead_count, preset_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
...
router.put('/:id', (req, res) => {
  const { name, host, port, api_key, enabled, bed_width, bed_depth, bed_height, filament_types, toolhead_count, preset_id } = req.body;
  const db = getDb();
  ...
  db.prepare(
    `UPDATE printers SET name=?, host=?, port=?, api_key=?, enabled=?,
     bed_width=?, bed_depth=?, bed_height=?, filament_types=?, toolhead_count=?, preset_id=?,
