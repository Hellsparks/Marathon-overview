...
export default function ArchivePage() {
    ...
    const { folders, loading: foldersLoading, error: foldersError, refresh: refreshFolders, createFolder, renameFolder, deleteFolder, moveFile, moveFolder } = useFolders('archive', '/api/projects');
    ...
    const [viewMode, setViewMode] = useState(() => localStorage.getItem('marathon_archive_view') || 'grid-large');
    const [sortBy, setSortBy] = useState('date');

    const updateViewMode = (mode) => {
        setViewMode(mode);
        localStorage.setItem('marathon_archive_view', mode);
    };
    ...
    const fetchData = async () => {
        ...
        try {
            const [projectsRes, filamentsData] = await Promise.all([
                ...
                fetch('/api/projects?status=archived').then(r => r.ok ? r.json() : Promise.reject(new Error('Failed to fetch projects'))),
                getFilaments().catch(() => [])
            ]);
    ...
    const handleDelete = async (id) => {
        if (!confirm('Are you sure you want to completely delete this archived project?')) return;
        try {
    ...
    const handleDeleteFolder = async (folder) => {
        if (confirm(`Delete folder "${folder.name}" and ALL its subfolders? Archived projects inside will be moved to root.`)) {
                    ...
                    <div className="folder-contents-area">
                                    ...
                                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        <thead>
                                            <tr style={{ borderBottom: '1px solid var(--border)', borderTop: '1px solid var(--border)', textAlign: 'left' }}>
                                                <th style={{ padding: '16px 8px', textTransform: 'uppercase', fontSize: '11px', fontWeight: 600, letterSpacing: '.05em', color: 'var(--text-muted)' }}>Archived Project</th>
                                                ...
                                                <th style={{ padding: '16px 8px', textTransform: 'uppercase', fontSize: '11px', fontWeight: 600, letterSpacing: '.05em', color: 'var(--text-muted)' }}>Date Archived</th>
                        ...
                        {currentFolders.length === 0 && currentProjects.length === 0 && (
                            <div className="empty-state">
                                <p>{currentFolderId ? 'This folder is empty.' : 'No archived projects.'}</p>
                                {!currentFolderId && <p style={{ fontSize: '13px', marginTop: '8px' }}>Completed or cancelled projects that are archived will appear here.</p>}
                            </div>
                        )}
                    </div>
                )}
